...
import { usePlayerDock } from '@/hooks/use-player-dock';
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const { isFloating } = usePlayerDock();
        ...
        <SidebarProvider>
          <PageHeaderProvider>
            ...
            <SidebarInset className="px-3 md:px-4">
              ...
              <div className={cn('flex-1 pt-3 md:pt-4', currentTrack || isRemote ? (isFloating ? 'pb-36 md:pb-8' : 'pb-36 md:pb-36') : 'pb-20 md:pb-8')}>{children}</div>
            </SidebarInset>
