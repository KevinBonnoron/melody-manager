...
function detectUrlSource(url: string): string | null {
  const t = url.trim();
  if (/^https?:\/\/((www\.|m\.|music\.)?youtube\.com|youtu\.be)(\/|$)/i.test(t)) {
    return 'youtube';
  }

  if (/^https?:\/\/(www\.)?soundcloud\.com(\/|$)/i.test(t)) {
    return 'soundcloud';
  }
...
export function SearchExperience({ variant = 'page', initialQuery = '', onNavigate }: Props) {
  ...
  // External search
  useEffect(() => {
    if (!trimmedQuery || scope === 'library' || urlMatch) {
      setExternalResults([]);
      return undefined;
    }
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const responses = await Promise.all(SEARCH_TYPES.map((type) => searchClient.search(trimmedQuery, type, { signal: controller.signal })));
        if (!cancelled) {
          setExternalResults(responses.flatMap((r) => r.results));
        }
      } catch {
        // Ignore abort errors
      } finally {
  ...
  // Held in a ref so the navigable list is not rebuilt on every render just to
  // keep hold of a function.
  ...
  const items = useMemo<Array<{ key: string; run: () => void; keepOpen?: boolean }>>(
    () => [
      ...
      ...visibleExternal.map((result) => ({ key: `external-${result.origin}`, run: () => handleAddRef.current(result), keepOpen: true })),
    ],
    [libraryResults, visibleExternal, navigate, playTrackWithContext],
  );
  ...
  useEffect(() => {
    if (activeKey) {
      listRef.current?.querySelector(`[data-key="${CSS.escape(activeKey)}"]`)?.scrollIntoView({ block: 'nearest' });
    }
  }, [activeKey]);
  ...
  return (
    ...
    <div
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === 'Enter' && items[cursor] && document.activeElement?.tagName !== 'BUTTON') {
          e.preventDefault();
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-primary-border bg-card">
        ...
        <input
          ref={inputRef}
          className="flex-1 bg-transparent outline-none text-[15px] min-w-0 placeholder:text-muted-foreground"
        ...
        />
        {query && (
          <button type="button" className="w-7 h-7 grid place-items-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/50" onClick={() => setQuery('')}>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        Album · {artistNames(album.artists, artistsById)} · {album.year}
                      </div>
                    </div>
                  </button>
                ))}
