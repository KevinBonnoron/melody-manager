...
export function PlaybackControls() {
  const { shuffle, repeatMode, queue, toggleShuffle, toggleRepeat, isPlaying, isLoading, currentTrack, playNext, playPrevious, togglePlayPause } = useMusicPlayer();
  const { isLiked, toggleLike } = useTrackRatings();
  const { canGoNext, canGoPrevious } = queueBounds({ queue, trackId: currentTrack?.id, repeatMode });
  return (
