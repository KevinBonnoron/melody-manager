...
export class CacheService {
  ...
  public constructor(options: CacheServiceOptions, logger?: ILogger) {
    this.options = options;
    this.log = logger ?? { debug: noop, error: noop, info: noop, warn: noop };
  ...
  public async downloadWithFn(cacheKey: string, downloadFn: () => Promise<string>): Promise<string> {
    ...
    const promise = (async () => {
      try {
        ...
        const stat = statSync(cachePath);
        this.cache.set(key, { path: cachePath, size: stat.size });
        this.log.debug(`[Cache] Stored ${stat.size} bytes for ${cacheKey}`);
        return cachePath;
      } finally {
        this.pendingDownloads.delete(key);
      }
