...
export function serializeAudioFilter(filter: AudioFilter): string[] {
  ...
  switch (filter.type) {
    ...
    case 'volume': {
      ...
      if (filter.precision) {
        filterString += `:precision=${filter.precision}`;
      }
      if (filter.eval) {
        filterString += `:eval=${filter.eval}`;
      }
      break;
    }
    case 'astats': {
      ...
      if (filter.length !== undefined) {
        params.push(`length=${filter.length}`);
      }
      if (filter.metadata !== undefined) {
        params.push(`metadata=${filter.metadata}`);
      }
      if (filter.reset !== undefined) {
        params.push(`reset=${filter.reset}`);
      }
    ...
    case 'loudnorm': {
      ...
      if (filter.TP !== undefined) {
        params.push(`TP=${filter.TP}`);
      }
      if (filter.LRA !== undefined) {
        params.push(`LRA=${filter.LRA}`);
      }
      if (filter.print_format) {
        params.push(`print_format=${filter.print_format}`);
      }
      if (filter.linear !== undefined) {
        params.push(`linear=${filter.linear ? 'true' : 'false'}`);
      }
