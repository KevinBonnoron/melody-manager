...
export async function handleSoundCloudStream(c: Context, options: StreamOptions, deps: PluginStreamDeps): Promise<Response> {
  ...
  if (hasTimeRange && !cached && !cacheService.isDownloadInProgress(cacheKey)) {
    cacheService.extractAndCache(streamUrl, cacheKey, startTime, endTime).catch((error) => {
      console.error(`[SoundCloud] Background extraction failed: ${error}`);
    });
  }
      ...
      if (cached) {
        return stream(c, async (s) => {
          ...
          proc.stderr.on('data', (data) => {
            ...
            if (line.includes('error') || line.includes('Error')) console.error(`[FFmpeg stderr] ${line}`);
          });
  ...
  if (hasTimeRange && !cached) {
    ...
    return stream(c, async (s) => {
      let cmd = ffmpeg();
      if (startTime !== undefined && startTime > 0) cmd = cmd.seekInput(startTime);
  ...
  if (cached) {
    ...
    if (range) {
      ...
      return stream(c, async (s) => {
        ...
        for await (const chunk of fileStream) await s.write(chunk);
      });
    }
    c.header('Content-Type', contentType);
  ...
  if (!response.ok) throw new Error('Failed to fetch stream from source');
