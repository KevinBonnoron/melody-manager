...
import { getAllAvailableProviderTypes } from './available-provider-types';
...
export function ProviderList({ title, description }: Props) {
  ...
  const allTypes = getAllAvailableProviderTypes(manifests);
  const availableToAdd = allTypes.filter(({ type }) => !configuredTypes.has(type));
      ...
      <DropdownMenuContent align="end" className="min-w-[12rem]">
        {availableToAdd.length === 0 ? (
          ...
          availableToAdd.map(({ type, category }) => {
            ...
            return <CreateProviderButton key={`${category}-${type}`} type={type} category={category} title={info?.title ?? type} description={info?.description ?? ''} />;
          })
        )}
      </DropdownMenuContent>
    </DropdownMenu>
