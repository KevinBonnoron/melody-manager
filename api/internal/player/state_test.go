...
func TestRemoveDropsATrackBehindThePlayheadWithoutMovingIt(t *testing.T) {
	s := State{List: []string{"a", "b", "c"}, Index: 2}.Sound().Remove("a", epoch)
	if s.Current() != "c" {
		t.Fatalf("current = %q", s.Current())
	}
}
...
func TestRemoveOfTheTrackBeingPlayedMovesToWhatTookItsPlace(t *testing.T) {
	s := State{List: []string{"a", "b", "c"}, Index: 1, Position: 30, PositionAt: epoch}.Sound().Remove("b", epoch)
	if s.Current() != "c" || s.Position != 0 {
		t.Fatalf("state = %+v", s)
	}
}

func TestRemoveOfTheOnlyTrackEmptiesTheList(t *testing.T) {
	s := State{List: []string{"a"}}.Sound().Remove("a", epoch)
	if len(s.List) != 0 || s.Current() != "" {
		t.Fatalf("state = %+v", s)
	}
}

func TestRemoveOfSomethingNotListedChangesNothing(t *testing.T) {
	s := listed("a", "b").Remove("z", epoch)
	if !reflect.DeepEqual(s.List, []string{"a", "b"}) {
		t.Fatalf("list = %v", s.List)
	}
}
...
func TestRemoveKeepsTheShuffledOrderConsistent(t *testing.T) {
	s := listed("a", "b", "c", "d").SetShuffle(true, reverse).Remove("c", epoch)
	for _, at := range s.Order {
		if at < 0 || at >= len(s.List) {
			t.Fatalf("order %v points outside a list of %d", s.Order, len(s.List))
		}
	}
	if len(s.Order) != 3 {
		t.Fatalf("order = %v", s.Order)
...
func TestClearEmptiesTheListButNotThePlayer(t *testing.T) {
	s := State{List: []string{"a", "b"}, Index: 1, Playing: true}.Sound().Clear()
	if len(s.List) != 0 {
		t.Fatalf("list = %v", s.List)
	}
...
func TestUnshufflingAfterAClearDoesNotRenameWhatIsPlaying(t *testing.T) {
	s := State{List: []string{"a", "b"}, Index: 1, Shuffle: true, Playing: true}.
		Sound().Clear().AddNext("z").SetShuffle(false, reverse)
	if s.Track != "b" {
		t.Fatalf("state = %+v", s)
	}
}

func TestShufflingAfterAClearDoesNotRenameWhatIsPlaying(t *testing.T) {
	s := State{List: []string{"a", "b"}, Index: 1, Playing: true}.Sound().Clear().AddNext("z").SetShuffle(true, reverse)
	if s.Track != "b" {
		t.Fatalf("state = %+v", s)
	}
}
