...
import type { ResolvedTrack, TrackMetadata, TrackProvider } from '@melody-manager/shared';
import type { FSWatcher } from 'chokidar';
import chokidar from 'chokidar';
import { parseFile } from 'music-metadata';
import { logger } from '../../lib/logger';
import type { ResolvedStream, WatchEventHandler, WatchProvider } from '../types';
...
export class LocalProvider implements WatchProvider {
  ...
  public async resolveTracks(_url: string, provider: TrackProvider): Promise<ResolvedTrack[]> {
  ...
  private async emitSync(_dirPath: string, provider: TrackProvider): Promise<void> {
    try {
      const tracks = await this.resolveTracks('', provider);
      tracks.sort((a, b) => (a.metadata?.trackNumber ?? 0) - (b.metadata?.trackNumber ?? 0));
      logger.info(`[local] Sync: ${tracks.length} tracks on disk`);
      this.onChange?.({ type: 'sync', tracks });
    } catch (error) {
      logger.error(`[local] Sync failed: ${error}`);
    }
  }
  ...
  private async emitAdded(filePath: string): Promise<void> {
    try {
      ...
      logger.error(`[local] Add failed: ${error}`);
    }
  }
  ...
  private async emitAddedDir(dirPath: string): Promise<void> {
    try {
      ...
      if (tracks.length > 0) {
        tracks.sort((a, b) => (a.metadata?.trackNumber ?? 0) - (b.metadata?.trackNumber ?? 0));
        logger.info(`[local] Directory added: ${dirPath} (${tracks.length} tracks)`);
        this.onChange?.({ type: 'added', tracks });
      }
    } catch (error) {
      logger.error(`[local] Dir add failed: ${error}`);
    }
  }

  private async scanDirectory(dirPath: string, tracks: ResolvedTrack[]): Promise<void> {
  ...
  private async extractTrack(filePath: string): Promise<ResolvedTrack | null> {
    try {
